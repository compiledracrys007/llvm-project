...
namespace mlir {
namespace memref {
#define GEN_PASS_DEF_DECOMPOSESUBVIEWCOPYPASS
...
struct MemRefRewritePattern : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T op,
    ...
    llvm::errs()<<"Decomposing subview copy op\n";
    return failure();
...
template <typename T>
struct SubViewToAffinePattern : public OpRewritePattern<memref::SubViewOp> {
  ...
  LogicalResult matchAndRewrite(memref::SubViewOp subViewOp,
    ...
    auto sourceType = dyn_cast<mlir::MemRefType>(source.getType());
    assert(sourceType && "expected memref type");
    auto resultType = dyn_cast<mlir::MemRefType>(subViewOp.getType());
    assert(resultType && "expected memref type");
    auto resultShape = resultType.getShape();
    // Get strides and offset of the source memref

    resultType = MemRefType::get(resultShape, rewriter.getF32Type());
    SmallVector<int64_t, 4> sourceStrides;
    int64_t sourceOffset;
    if (failed(sourceType.getStridesAndOffset(sourceStrides, sourceOffset)))
      return failure();

    // Create affine expressions for the access
    SmallVector<AffineExpr, 4> dims;
    dims.reserve(sourceType.getRank());
    
    // Create dimensional expressions for each index
    for (unsigned i = 0; i < sourceType.getRank(); ++i) {
      dims.push_back(rewriter.getAffineDimExpr(i));
    }

    // Build affine map for the access
    auto map = AffineMap::get(sourceType.getRank(), 0, dims, 
                             rewriter.getContext());

    // Create a new memref with the same type as the result
    ...
    // Create affine loops to copy data
    SmallVector<int, 4> lbs, ubs, steps;
    assert(offsets.size() == sizes.size() &&
           sizes.size() == strides.size() &&
           "offsets, sizes, and strides must have the same size");
    for(auto i = 0; i < sizes.size(); ++i) {
        lbs.push_back(offsets[i]);
        ubs.push_back(sizes[i]);
        steps.push_back(strides[i]);
    }
    SmallVector<Value> ivs;
    affine::AffineForOp lastLoop; // Keep track of the last loop

    for (unsigned i = 0; i < sourceType.getRank(); ++i) {

        auto loop = rewriter.create<affine::AffineForOp>(
            loc, lbs[i], ubs[i], steps[i]);
      
      lastLoop = loop; // Store the current loop
      rewriter.setInsertionPointToStart(loop.getBody());
      ivs.push_back(loop.getInductionVar());
      
      if (i == sourceType.getRank() - 1) {
        // Create load and store operations in innermost loop
        auto loadOp = rewriter.create<affine::AffineLoadOp>(
            loc, source, map, ivs);
        
        rewriter.create<affine::AffineStoreOp>(
            loc, loadOp.getResult(), allocOp.getResult(), map, ivs);
      }
    }

    // Reset insertion point to after the outermost loop
    rewriter.setInsertionPointAfter(lastLoop);

    rewriter.replaceOp(subViewOp, allocOp.getResult());
    return success();
  }
};

struct DecomposeSubviewCopyPass
    : public mlir::memref::impl::DecomposeSubviewCopyPassBase<DecomposeSubviewCopyPass> {
  using Base::Base;
  ...
  void runOnOperation() override {
    ...
    llvm::errs()<<"Populating decompose subview copy patterns\n";

    memref::populateDecomposeSubviewCopyPatterns(patterns);
...
void memref::populateDecomposeSubviewCopyPatterns(RewritePatternSet &patterns) {
  patterns.insert<MemRefRewritePattern<memref::SubViewOp>>(
      patterns.getContext());
    patterns.insert<SubViewToAffinePattern<memref::SubViewOp>>(
      patterns.getContext());
}
